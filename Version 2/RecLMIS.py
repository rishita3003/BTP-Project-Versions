...
class RecLMIS(nn.Module):
    def __init__(self, global_config, config, n_channels=3, n_classes=1, img_size=224, vis=False):
        ...
        #change
        # OPTION 1: Replace entirely with ControlGAN
        self.interact = EnhancedInteractor(config, vis, img_size=14, channel_num=512, patch_size=1, embed_dim=512)

        # OPTION 2: Keep both for comparison (add this flag to config)
        if config.use_controlgan_attention:
            self.interact = EnhancedInteractor(config, vis, img_size=14, channel_num=512, patch_size=1, embed_dim=512)
        else:
    ...
    def cond_cons_loss(self, text_feat, text_mask, img_feat, text_weight=None, img_weight=None):
        ...
        # change - added the below snippet
        if hasattr(self.interact, 'spatial_attention'):
          # Get attention weights from ControlGAN module
          _, controlgan_attn = self.interact.spatial_attention(
              img_feat.transpose(1, 2).view(-1, 512, 14, 14),
              text_feat,
              text_mask
          )
          # Use these weights instead of or in combination with existing weights
          if img_weight is None:
              img_weight = controlgan_attn.mean(dim=-1)  # Average over text tokens
          if text_weight is None:
              text_weight = controlgan_attn.mean(dim=1)   # Average over visual positions
    ...
    def forward(self, images, masks, text_token, text_mask, mode="train"):
        ...
        #change
        # Pass text_mask to enhanced interactor
        ...
        #change - added the initializations
        img_weight = None
        text_weight = None
        if self.aux:
            try:
