...
class RecLMIS(nn.Module):
    ...
    # changee
    def reconstructor(self, text_feat4, text_feat, text_mask,
                  img_feat4, img_feat, text_feat_clip,
    ...
    def forward(self, images, masks, text_token, text_mask, mode="train"):
        ...
        # changee 1
        text_feat4_4rec, img_feat4_4rec, Wpoi, Wwoi = self.interact(img_feat4, text_feat) 
        
        assert Wpoi.dim() == 2, Wpoi.shape       # [B, P]
        assert Wwoi.dim() == 2, Wwoi.shape       # [B, L]
        assert img_feat4_4rec.shape[1] == Wpoi.shape[1]
        assert text_feat4_4rec.shape[1] == Wwoi.shape[1]
        ...
        if self.aux:
            try:
                # changee
                rec_loss_dic, img_weight, text_weight = self.reconstructor(text_feat, text_feat4_4rec, text_mask, img_feat4, img_feat4_4rec, text_feat_clip, img_weight = Wpoi, text_weight = Wwoi)
